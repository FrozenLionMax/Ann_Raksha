...
export default function Dashboard() {
  ...
  const impactCards = [
    { label: 'Meals Provided', value: impact.mealsProvided || 0, icon: Flame, color: 'text-orange-400', bg: 'bg-orange-500/10' },
    { label: 'CO₂ Prevented', value: `${(impact.co2Saved || 0).toFixed(1)} kg`, icon: Leaf, color: 'text-green-400', bg: 'bg-green-500/10' },
    { label: 'Water Saved', value: `${((impact.waterSaved || 0) / 1000).toFixed(1)}K L`, icon: Droplets, color: 'text-blue-400', bg: 'bg-blue-500/10' },
    { label: 'Total Donations', value: impact.totalDonations || 0, icon: Package, color: 'text-emerald-400', bg: 'bg-emerald-500/10' },
  ];
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          >
            <div className="flex items-center justify-between mb-3">
              ...
              <button onClick={() => navigate('/my-donations')} className="text-sm text-emerald-400 hover:text-emerald-300 font-semibold">
                View All →
              </button>
            </div>
              ...
              {recent.length === 0 ? (
                <div className="p-10 text-center">
                  ...
                  <p className="text-sm text-slate-500 mt-1">
                    {isNgo ? "Browse donations to get started" : "Create your first donation"}
                  </p>
                </div>
              ) : (
